...
class Trainer():
    ...
    def build_scheduler(self, optimizer, config_lr_scheduler):
        if not config_lr_scheduler["status"]:
            return None
        scheduler_func = lambda epoch: lr_lambda_update(epoch, config_lr_scheduler)
    ...
    def train(self):
        ...
        while self.current_epoch <= self.max_epochs:            
            ...
            for batch_id, batch in tqdm(enumerate(self.train_loader), desc="Iterating through train loader"):
                self.writer.LOG_INFO(f"Training batch: {batch_id + 1} - Epoch: {self.current_epoch}")
    ...
    def load_model(self, model_path):
        ...
        if self.lr_scheduler is not None and 'scheduler_state_dict' in checkpoint and checkpoint['scheduler_state_dict'] is not None:
            try:
                ...
                # fallback: advance scheduler to current_epoch
                self.writer.LOG_INFO("Warning: failed to load scheduler state; advancing scheduler to epoch")
